        ...
        <?php if (!empty($data_berita)): ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php foreach ($data_berita as $news): ?>
                    ...
                    ?>
                    <a href="<?= site_url('base/berita/' . $news->id_berita); ?>" class="block bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 card-hover focus:outline-none focus:ring-2 focus:ring-amber-500" aria-label="<?= html_escape($judul_berita); ?>">

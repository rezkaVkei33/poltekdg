...
<main class="admin-landing container my-4">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-semibold" style="color: #023c5e;" id="pageTitle">DATA RENCANA STRATEGIS</h2>
        </div>
        <div>
            <a class="btn btn-primary bg-gradient-info" href="<?= base_url('renstra/tambah_renstra'); ?>">
        ...
        <div class="table-responsive-custom">
            <table id="dataTable" class="table table-hover align-middle" style="width:100%">
                ...
                <?php if(!empty($renstra)) : ?>
                    <?php foreach($renstra as $data): ?>
                        ...
                        <td data-label="Dokumen">
                           <?php if (!empty($data->file_upload)): ?>
                            ...
                            <br>
                            <a href="<?= base_url('uploads/renstra/' . $data->file_upload); ?>" download class="btn btn-success">
                        ...
                        <td class="text-center" data-label="Aksi">
                            <a href="<?= base_url('renstra/ubah_renstra/' . $data->id_renstra); ?>" class="btn btn-warning btn-sm text-white"> 
                            <i class="bi bi-pencil-square me-1"></i>
                            Ubah
                            </a>
                            <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#hapus_renstra-<?= $data->id_renstra; ?>"> 
                            <i class="bi bi-trash me-1"></i>
                            Hapus
                            </button>

                          <!-- Modal Hapus Sambutan -->
                        <div class="modal fade" id="hapus_renstra-<?= $data->id_renstra; ?>" tabindex="-1" aria-labelledby="modalLabel>" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                  <div class="modal-header bg-danger text-white">
                                    <h5 class="modal-title text-white" id="modalLabel">HAPUS RENSTRA</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                                  </div>
                                  <div class="modal-body text-center">
                                    <p>Apakah Anda yakin ingin <br>
                                    menghapus File Renstra dari <strong><?= $data->judul; ?></strong>?</p>
                                  </div>
                                  <div class="modal-footer">
                                    <a href="<?= base_url('renstra/hapus_renstra/' . $data->id_renstra); ?>" class="btn btn-danger">Ya, Hapus</a> 
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="text-center text-danger ">Tidak ada data Renstra.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
              </table>
